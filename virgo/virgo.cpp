...
static HWND *stb__sbgrowf(HWND *arr, unsigned increment, unsigned itemsize)
{
	unsigned int dbl_cur = arr ? 2 * stb__sbm(arr) : 0;
	unsigned int min_needed = sb_count(arr) + increment;
	unsigned int m = dbl_cur > min_needed ? dbl_cur : min_needed;
	unsigned int *p;
	if (arr) {
		p = (unsigned int*)(HeapReAlloc(GetProcessHeap(), 0, stb__sbraw(arr),
			itemsize * m + sizeof(unsigned) * 2));
	}
	else {
		p = (unsigned int*)(HeapAlloc(GetProcessHeap(), 0, itemsize * m + sizeof(unsigned) * 2));
	}
	if (p) {
		if (!arr) {
			p[1] = 0;
...
static void trayicon_init(Trayicon *t)
{
	...
	t->nid.hIcon = trayicon_draw(t, (char *)"1", 1);
